...
import httpx
from API_keys.apikey import openAIKey
...
class OpenAI:
    ...
    async def run_request(prompt: str) -> str:
        ...
        try:
            ...
            #body = OAIRequestWithUserInfo(prompt=prompt, email='documentor-playground@textql.com')
            #print(prompt)
            #data = json.dumps(body.__dict__)
            #print(f"Using TextQL API with user info {'documentor-playground@textql.com'}")
            #print(f"Data {data}")
